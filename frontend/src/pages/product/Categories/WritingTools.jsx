...
const WritingTools = () => {
  ...
  const getstationerysubcategory = (subcategory) => {
    if (!products || products.length === 0) return [];
    return products.filter((product) => product.subcategory === subcategory)
      .reverse();
  }
  const displayProducts = getstationerysubcategory();
  if (displayProducts.length === 0) return <div>No products found</div>;

  const stationerysubcategory = getstationerysubcategory("writing-tools");
  ...
  return (
    <div className="p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        {stationerysubcategory.map((product) => (
          <div
            key={product.id}
            className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm 
                     transition-transform duration-200 hover:-translate-y-1 hover:shadow-md"
          >
            <div className="h-36  mb-3 flex items-center justify-center text-gray-500">
