import { useNavigate } from "react-router-dom";

const FilingOrganization = () => {
  const navigate = useNavigate();

  const products = [
    {
      id: 11,
      name: "File Folder Set",
      price: "$6.99",
      image: "folder1.jpg",
      description: "Pack of 25 manila file folders with tabs"
    },
    {
      id: 12,
      name: "3-Ring Binder",
      price: "$4.99",
      image: "binder1.jpg",
      description: "Durable 1-inch 3-ring binder with clear pockets"
    },
    {
      id: 13,
      name: "Document Organizer",
      price: "$15.99",
      image: "organizer1.jpg",
      description: "Desktop document organizer with multiple compartments"
    },
    {
      id: 14,
      name: "Paper Clips Box",
      price: "$2.99",
      image: "clips1.jpg",
      description: "Box of 100 standard paper clips"
    },
    {
      id: 15,
      name: "Hanging Folders",
      price: "$8.99",
      image: "hanging1.jpg",
      description: "Set of 20 hanging folders for filing cabinets"
    },
    {
      id: 16,
      name: "Accordion File",
      price: "$12.99",
      image: "accordion1.jpg",
      description: "Expandable accordion file with 13 pockets"
    }
  ];

  return (
    <div className="p-5">
      {/* Header */}
      <div className="mb-5">
        <button
          onClick={() => navigate("/")}
          className="px-4 py-2 bg-gray-500 text-white border-none rounded cursor-pointer mb-4 
                   hover:bg-gray-600 transition-colors"
        >
          ← Back to Categories
        </button>
        <h1 className="text-3xl font-bold text-gray-800 mb-2">📂 Organization & Filing</h1>
        <p className="text-gray-600">Keep your documents organized with our filing solutions</p>
      </div>

      {/* Products Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {products.map((product) => (
          <div
            key={product.id}
            className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm 
                     transition-transform duration-200 hover:-translate-y-1 hover:shadow-md"
          >
            <div className="h-36 bg-gray-100 rounded mb-3 flex items-center justify-center text-gray-500">
              Product Image
            </div>
            <h4 className="text-lg font-semibold mb-2 text-gray-800">{product.name}</h4>
            <p className="text-sm text-gray-600 mb-3">{product.description}</p>
            <p className="text-xl font-bold text-blue-600 mb-3">
              {product.price}
            </p>
            <button className="w-full py-2 bg-blue-500 text-white border-none rounded cursor-pointer 
                             hover:bg-blue-600 transition-colors">
              Add to Cart
            </button>
          </div>
        ))}
      </div>
    </div>
  );
};

export default FilingOrganization;